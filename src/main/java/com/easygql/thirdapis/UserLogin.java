    ...
    CompletableFuture.runAsync(
        () -> {
          ...
          DataSelecter dataSelecter =
              GraphQLCache.getEasyGQL(thirdAPIInput.getSchemaData().getSchemaid())
                  .getObjectDaoMap()
                  .get(GRAPHQL_USER_TYPENAME)
                  .getDataselecter();
          HashMap nameEqMap = new HashMap();
          nameEqMap.put(GRAPHQL_FILTER_EQ_OPERATOR, username);
          HashMap passwordEqMap = new HashMap();
          passwordEqMap.put(GRAPHQL_FILTER_EQ_OPERATOR, securePassword);
          ...
          ObjectTypeMetaData objectTypeMetaData =
              thirdAPIInput.getSchemaData().getObjectMetaData().get(GRAPHQL_USER_TYPENAME);
          ...
          ;
          dataSelecter
              .getSingleDoc(filterMap, selectionMap)
              .whenCompleteAsync(
                  (queryResult, queryEx) -> {
                    if (null != queryEx) {
